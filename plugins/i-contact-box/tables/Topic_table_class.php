...
class Topic_table_class extends WP_List_Table {
    ...
    function column_topic($item){ //Table column topic
    ...
    function get_columns(){ //Table columns
        $columns = array(
            ...
            'topic' => 'Topic', //Table column topic
            'email' => 'Email', //Table column email
            'stat' => 'Status' //Table column stat
        );
        return $columns;
    }
    function get_sortable_columns(){
        $sortable_columns = array(
            'topic' => array('topic', true), //Table column topic
            'email' => array('email', true), //Table column topic
            'stat' => array('status', false) //Table column stat
        );
    ...
    function prepare_items(){
        ...
        $orderby = (isset($_REQUEST['orderby']) && in_array($_REQUEST['orderby'], array_keys($this->get_sortable_columns()))) ? $_REQUEST['orderby'] : 'topic';
        ...
        if(isset($_REQUEST['s'])){
            ...
            $sql_query = $wpdb->prepare("SELECT * FROM $table_name WHERE CONCAT_WS('',`topic`,`email`, `stat`) LIKE '%$search_key%' ORDER BY $orderby $order LIMIT $per_page OFFSET $paged");
        } else {
...
function icb_contact_page_handler(){
    global $wpdb;
    $table = new Topic_table_class();
    $table->prepare_items();
    $message = '';
...
<div class="wrap">
    ...
    <?php echo $message; ?>
    <form id="topics-table" method="GET">
