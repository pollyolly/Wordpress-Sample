...
class Message_table_class extends WP_List_Table {
    ...
    function prepare_items(){
        ...
        // WHERE CONCAT(`fullname`,`email`, `subject`, `message`) LIKE %$search_key%
        // $search_key = isset($_REQUEST['s']) ? $_REQUEST['s'] : '';
        $sql_query = null;
        if(isset($_REQUEST['s'])){
            $search_key = $_REQUEST['s'];
            $sql_query = $wpdb->prepare("SELECT * FROM $table_name WHERE CONCAT_WS('',`fullname`,`email`, `subject`, `message`) LIKE '%$search_key%' ORDER BY $orderby $order LIMIT $per_page OFFSET $paged");
        } else {
